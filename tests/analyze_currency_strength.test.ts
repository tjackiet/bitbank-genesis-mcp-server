...
import analyzeCurrencyStrength, { toolDef } from '../tools/analyze_currency_strength.js';
...
function indicatorsOk(rsi: number, sma25: number, latestClose: number) {
	return {
		...
		data: {
			...
			normalized: [{ close: latestClose, isoTime: '2024-01-01T00:00:00.000Z' }],
		},
...
describe('analyze_currency_strength', () => {
	...
	it('正常系: 出来高上位 N 件をスコア順にランキングする', async () => {
		...
		expect(res.data.rankings.map((item) => item.rank)).toEqual([1, 2, 3]);
	...
	it('重複 pair が含まれる場合は同一銘柄を重複ランキングしないべき', async () => {
		...
		assertOk(res);
		expect(new Set(res.data.rankings.map((item) => item.pair)).size).toBe(res.data.rankings.length);
	});
