...
import { buildTickersJpyRankedText, buildTickersJpyItemsText, type NormalizedTicker } from '../src/handlers/getTickersJpyHandler.js';
...
describe('buildTickersJpyRankedText', () => {
  ...
  it('ランキング行に番号・ペア名・変化率・価格・出来高を含む', () => {
    const text = buildTickersJpyRankedText(10, [
  ...
  it('changeN が負の場合はマイナス符号が付く', () => {
    const text = buildTickersJpyRankedText(10, [
      makeTicker('eth_jpy', { changeN: -3.5 }),
    ], 'change24h', 'desc', 5);
  ...
  it('changeN が null の場合は n/a', () => {
    const text = buildTickersJpyRankedText(10, [
      makeTicker('xrp_jpy', { changeN: null }),
    ], 'change24h', 'desc', 5);
