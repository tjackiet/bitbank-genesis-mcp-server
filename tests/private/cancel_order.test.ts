...
describe('cancel_order', () => {
	it('キャンセル成功時に注文情報を返す', async () => {
		...
		const result = await cancelOrder({ pair: 'btc_jpy', order_id: 2001 });
	...
	it('売注文のキャンセルで「売」ラベルが表示される', async () => {
		...
		const { default: cancelOrder } = await import('../../tools/private/cancel_order.js');
		const result = await cancelOrder({ pair: 'eth_jpy', order_id: 2001 });
	...
	it('注文が見つからない場合（50009）に適切なエラーメッセージ', async () => {
		...
		const { default: cancelOrder } = await import('../../tools/private/cancel_order.js');
		const result = await cancelOrder({ pair: 'btc_jpy', order_id: 99999 });
