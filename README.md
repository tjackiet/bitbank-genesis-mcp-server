...
[![npm version](https://img.shields.io/npm/v/@tjackiet/bitbank-mcp)](https://www.npmjs.com/package/@tjackiet/bitbank-mcp)
...
#### npm パッケージを使う方法（インストール不要）

Node.js さえあれば、git clone なしで利用できます。
...
> **npx を使う場合**: 上記の npm パッケージ設定を `claude_desktop_config.json` に追加するだけで完了です。以下はソースコードから直接起動する場合の設定です。
...
Claude Desktop 以外の MCP クライアントでも、同じ npm パッケージを使って登録できます。

#### Claude Code

```bash
claude mcp add bitbank -- npx -y @tjackiet/bitbank-mcp
```
