...
提供される情報は情報提供のみを目的としており、投資助言・投資代理業に該当するものではありません。投資に関する判断はご自身の責任で行ってください。

## クイックスタート

### 前提条件
- **Node.js 18 以上**（[公式サイト](https://nodejs.org/) からインストール。`node -v` で確認できます）
- Git（`git clone` で取得するため）
...
|------|---------|------------|-----------|
| キー未設定 | 29（Public のみ） | 8 | 価格取得・テクニカル分析・チャート生成・バックテスト |
| キー設定済み | 29 + 16 = **45** | 8 + 1 = **9** | 上記 + 資産確認・注文・ポートフォリオ分析 |
