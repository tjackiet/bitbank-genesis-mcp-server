...
3. **方式 A → B の切り替え** — `command: "npx"` で動かない場合は、GUI アプリ
   から `npx` が解決できていない可能性が高い。`which node` の絶対パスを
   `command` に指定する方式 B へ切り替える（Step 3 参照）。
4. **パスの確認** — 設定ファイルの `command` / `args` / `workingDirectory` が
   すべて絶対パスで、実在するか（方式 B の場合は Node バージョン更新で絶対
   パスが変わる点にも注意）。
5. **ログの確認** — 設定の `env` に `"LOG_LEVEL": "debug"` を追加して再起動し、
   クライアントのログを確認する。
...
設定方法は 2 通りある。**まず方式 A を提示し、動かなければ方式 B に切り替える。**

#### 方式 A — `npx` 経由（推奨）

Node.js のバージョンアップで設定書き換えが不要。nvm / volta ユーザーにも安定。
...
方式 A で「サーバーに接続できません」「`npx` not found」が出るクライアント（GUI アプリから `npx` を解決できないケース）で使う。`which node` の結果を `command` に指定する。
...
nvm / volta ユーザーは Node.js バージョンアップでパスが変わるため、方式 A を優先する。

注: npm パッケージ（`@tjackiet/bitbank-mcp`）は未公開なので `npx -y @tjackiet/bitbank-mcp` は現状使わない。上記方式 A の `npx tsx ...` は tsx 実行用であり、この制約とは無関係。
