...
既存の `command` / `args` は `references/client-configs.md` のソースコード起動
設定（方式 A / B）をそのまま使い、`env` に API キーを追加する形になる。以下の例
は方式 A（推奨）を示す。方式 B の場合は `command` を `<ABS_NODE_PATH>`、`args`
を `[tsx/dist/cli.mjs, server.ts]` の 2 要素に置き換える。
