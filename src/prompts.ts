...
- テキスト: var(--color-text-primary), var(--color-text-secondary)
- フォント: var(--font-sans), var(--font-mono)
- **フォントサイズ（重要）**: 本文・テーブル・ラベル等のテキストは **最低 14px**（0.875rem）以上にする。補足テキストでも 12px 未満にしない。SVG 内のテキスト（軸ラベル・値ラベル）も **最低 12px** 以上。全体的に読みやすさを最優先する
...
- ダークテーマ
  ...
  - テキスト: #eaeaea
  - アクセント: #0f3460
  - フォントサイズ: 本文・テーブル・ラベル等は最低 14px 以上。補足テキストでも 12px 未満にしない。SVG 内テキストも最低 12px 以上
