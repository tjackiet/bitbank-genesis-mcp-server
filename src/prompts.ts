...
1. analyze_my_portfolio(include_pnl=true, include_technical=true, include_deposit_withdrawal=true)
   → 口座全体の保有資産・月初比/年初比パフォーマンス・評価損益・入出金ベースのリターン・テクニカル分析を一括取得
...
【指標の定義と優先順位】
本レポートの主指標は **口座評価額の期間比較**:
- **年初比パフォーマンス**: 当年 1/1 00:00 JST 時点の口座評価額 vs 現在の口座評価額
- **月初比パフォーマンス**: 当月 1日 00:00 JST 時点の口座評価額 vs 現在の口座評価額
- 期間基準は JST（日本標準時）固定
- カードの主表示は **単純増減（change_jpy / change_pct）** — ユーザーが直感的に把握できる数字
- 期間中に入出金がある場合は補足として「入出金調整後（adjusted_change_jpy）」も表示
- 入出金の扱い:
  - **出金元本**: 外部フローとして net_flow_jpy に含め、adjusted_change から除外
  - **出金手数料**: コストとして withdrawal_fee_jpy に分離し、adjusted_change_jpy に残る
  - 暗号資産の入出庫は現在価格で仮評価

補助指標として以下も表示するが、主役ではない:
- 現在の口座合計評価額（total_jpy_value）
- 評価損益（unrealized_pnl）= 現在保有分の含み損益。約定履歴の平均取得単価ベース
- 全履歴実現損益（total_realized_pnl）= 口座開設以降の累計（必要なら表示）
- 口座全体リターン（account_return）= 入出金ベースの概算

※ 評価損益は参考値であり、税額計算の確定値ではない
...
- **年初比パフォーマンス**（yearly_performance）— 主指標として目立つ位置に配置
  - 年初評価額 → 現在評価額の変化を表示
  - **主表示**: 単純増減（change_jpy / change_pct）を大きく表示
  - 入出金がある場合は補足として「入出金調整後」（adjusted_change_jpy）を小さく表示
  - 純入出金（元本）・出金手数料があれば補足行で表示
- **月初比パフォーマンス**（monthly_performance）— 主指標として目立つ位置に配置
  - 月初評価額 → 現在評価額の変化を表示
  - 同様に単純増減を主表示、入出金調整後は補足
- 合計評価損益（total_unrealized_pnl）— 補助指標として小さめに表示
- 口座全体リターン（account_return_jpy, account_return_pct）— available 時のみ、補助指標として表示
- プラスは緑、マイナスは赤で色分け
...
- 銘柄に realized_pnl がある場合は同じテーブル内に列を追加して表示してよい
    ...
    <section class="bg-card rounded-lg p-6">
      ...
      <div class="grid grid-cols-2 gap-4 text-center mb-4">
        <div class="bg-accent rounded-lg p-3">
          <p class="text-gray-400 text-xs">年初比</p>
          <p class="text-gray-400 text-xs">{yearly_start_value}円 → {yearly_current_value}円</p>
          <!-- 主表示: 単純増減（change_jpy / change_pct） -->
          <p class="{yearly_color} text-xl font-bold">{yearly_change}</p>
          <!-- 入出金がある場合の補足: 入出金調整後・純入出金・出金手数料 -->
          <p class="text-gray-500 text-xs">{yearly_adjusted_note}</p>
          <p class="text-gray-500 text-xs">{yearly_net_flow_note}</p>
          <p class="text-gray-500 text-xs">{yearly_fee_note}</p>
        </div>
        <div class="bg-accent rounded-lg p-3">
          <p class="text-gray-400 text-xs">月初比</p>
          <p class="text-gray-400 text-xs">{monthly_start_value}円 → {monthly_current_value}円</p>
          <!-- 主表示: 単純増減（change_jpy / change_pct） -->
          <p class="{monthly_color} text-xl font-bold">{monthly_change}</p>
          <!-- 入出金がある場合の補足: 入出金調整後・純入出金・出金手数料 -->
          <p class="text-gray-500 text-xs">{monthly_adjusted_note}</p>
          <p class="text-gray-500 text-xs">{monthly_net_flow_note}</p>
          <p class="text-gray-500 text-xs">{monthly_fee_note}</p>
        </div>
      </div>
      <!-- 補助指標 -->
      ...
      <p class="text-gray-500 text-xs text-center mt-2">期間基準: JST（日本標準時） / 出金元本は外部フローとして除外、出金手数料はコストとして残す</p>
    </section>
    ...
    <section class="bg-card rounded-lg p-6">
      ...
      <div class="text-sm text-gray-400 space-y-2">
        ...
        <ul class="list-disc list-inside space-y-1 ml-2">
          ...
          <li>入出金がある場合、調整後増減 = 単純増減 - 純入出金（市場変動 + 出金手数料コスト）</li>
          <li>出金元本は外部フロー（performance から除外）、出金手数料はコスト（performance に残る）</li>
...
- holdings 内に realized_pnl がある銘柄は、同テーブル内に実現損益列を追加して表示
