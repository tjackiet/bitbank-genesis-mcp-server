...
- viewBox="0 0 600 150"
- 描画エリア（ピクセル座標）: x=10〜590（幅580px）、y=20〜120（高さ100px）。これはSVG内部のピクセル座標であり、表示するデータ値ではない
- X 座標: 各点を描画エリア幅で均等配置（x = 10 + i / (points.length - 1) * 580）
- Y 座標: value_jpy を min-max スケーリングで描画エリア内に反転マッピング（上が高い値）
  - yScale = (value - minValue) / (maxValue - minValue)  ※ min==max なら 0.5
  - y = 120 - yScale * 100  （120がエリア下端、20がエリア上端）
- \`<polyline>\` で折れ線、\`<polygon>\` で半透明の面塗り、始点・終点に \`<circle>\` マーカー
- 線の色: 最終点 ≥ 最初の点なら #4ade80（緑）、そうでなければ #f87171（赤）
- X 軸ラベル: 左に「月初」/「年初」、右に「現在」（y=140 付近に配置）
- Y 軸ラベル: **実データの JPY 値**（¥XX,XXX 形式）を薄い色（text-gray-500 等）で表示。上端に maxValue、下端に minValue を配置。**ピクセル座標（10, 110 等）を表示してはいけない**
    ...
    <section class="bg-card rounded-lg p-6">
      ...
      <div class="mb-6">
        ...
        <svg viewBox="0 0 600 150" class="w-full">
          <!-- 描画エリア: x=10〜590, y=20〜120（ピクセル座標。表示値ではない） -->
          <!-- X: 各点を均等配置 (x = 10 + i / (N-1) * 580) -->
          <!-- Y: value_jpy を min-max スケーリングで y=20〜120 に反転マッピング -->
          <!-- polyline: 折れ線, polygon: 半透明面塗り, circle: 始点・終点 -->
          <!-- 線色: series末尾 >= series先頭 → #4ade80, else → #f87171 -->
          <!-- X軸ラベル(y≈140): 左「月初」右「現在」 -->
          <!-- Y軸ラベル: 上端に ¥{maxValue}, 下端に ¥{minValue}（実データのJPY値を表示） -->
        </svg>
      ...
      <div>
        ...
        <svg viewBox="0 0 600 150" class="w-full">
          <!-- 月次グラフと同じ描画ロジック -->
          <!-- Y軸ラベル: ¥{maxValue} / ¥{minValue}（実データのJPY値） -->
        </svg>
        <div class="flex justify-between text-gray-500 text-xs mt-1">
