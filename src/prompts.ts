...
- 始点・終点に値ラベル（「月初: XX円」「現在: XX円」等）

**デザインゴール（ピクセル座標は指定しない。見た目の品質を優先して自由に設計してよい）:**
- **余白**: グラフ描画エリアの上下左右に十分なパディングを取り、ラインやラベルが見切れないようにする。特に **上部は値ラベルの高さ分（最低 30px 相当）を追加確保** する
- **ラベル配置（重なり防止 — 重要）**: 値ラベルは折れ線・塗りつぶし領域と **必ず離して配置** する。具体的には:
  - 始点ラベル（「月初: XX円」等）と終点ラベル（「現在: XX円」等）は、**データ点の真上ではなくグラフ描画エリアの上部余白**（折れ線より上の固定位置）に配置する
  - 始点ラベルは左寄せ（text-anchor: start）、終点ラベルは右寄せ（text-anchor: end）で、水平方向も重ならないようにする
  - ラベルと折れ線の間に **最低 12px 以上の垂直ギャップ** を確保する
  - Y 軸の値域（min〜max）に対して上下に 10〜15% のマージンを追加し、折れ線が描画エリア上端に張り付かないようにする
- **二色塗り分け**: 基準値（期初の value_jpy）を水平ラインとして、基準値以上の区間は **緑（#4ade80）** の線＋半透明緑の塗り、基準値を下回る区間は **赤（#f87171）** の線＋半透明赤の塗りで描画する。SVG の \`<clipPath>\` 等を活用して塗り分ける
- **Y 軸**: 実データの JPY 値（¥XX,XXX 形式）をグラフ左側に表示。ピクセル座標を表示してはいけない
- **X 軸**: 左に「月初」/「年初」、右に「現在」
...
- **評価損益**（unrealized_pnl）= 現在評価額 − 取得原価。現在保有分の含み損益（未実現）
- **損益率**（unrealized_pnl_pct）= 評価損益 ÷ 取得原価 × 100。**全取引履歴ベースの移動平均法で算出した取得原価に対する含み損益の割合**であり、特定期間のリターンではない
...
- **評価損益** = 現在評価額 − 取得原価（含み損益。未実現）
- **損益率** = 評価損益 ÷ 取得原価 × 100（%）。全約定履歴から移動平均法で算出した取得原価が基準。特定期間（月初来・年初来等）のリターンではなく、口座開設以降の全取引を通じた保有コストに対する現在の含み損益の割合です
  ...
  <div class="max-w-3xl mx-auto space-y-6">
    ...
    <section class="bg-card rounded-lg p-6">
      ...
      <div class="mb-6">
        ...
        <svg viewBox="..." class="w-full"><!-- monthly_equity_series の全点を描画 --></svg>
      </div>
      ...
      <div>
        ...
        <svg viewBox="..." class="w-full"><!-- yearly_equity_series の全点を描画 --></svg>
      </div>
    </section>
