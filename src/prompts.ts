...
- viewBox="0 0 600 150"
- 描画エリア x: 10〜590、y: 10〜110
- X 座標: 各点を描画エリア幅で均等配置（i / (points.length - 1) * width）
- Y 座標: value_jpy を min-max スケーリングで反転（上が高い値）
- \`<polyline>\` で折れ線、\`<polygon>\` で半透明の面塗り、始点・終点に \`<circle>\` マーカー
- 線の色: 最終点 ≥ 最初の点なら #4ade80（緑）、そうでなければ #f87171（赤）
- X 軸左に「月初」/「年初」、右に「現在」のラベル
- Y 軸の max / min を薄いテキストで表示するとなお良い
  ...
  <div class="max-w-3xl mx-auto space-y-6">
    ...
    <section class="bg-card rounded-lg p-6">
      ...
      <div class="mb-6">
        ...
        <!-- X: 各点を均等配置 (i / (N-1) * 580 + 10) -->
        <!-- Y: value_jpy を min-max スケーリング、上が高い値 -->
        <svg viewBox="0 0 600 150" class="w-full">
          <!-- polyline: 折れ線 -->
          <!-- polygon: 半透明面塗り（polyline + 右下 + 左下） -->
          <!-- circle: 始点・終点マーカー -->
          <!-- 線色: series末尾 >= series先頭 → #4ade80, else → #f87171 -->
          <!-- X軸ラベル: 左に「月初」、右に「現在」 -->
          <!-- Y軸: min/max を薄いテキストで表示 -->
        </svg>
