...
**実装方針（重要 — SVG 直書き禁止）:**
SVG の軸ラベル・グリッド線・折れ線の座標をハードコードした SVG タグとして直接出力してはならない。
LLM のストリーミング出力は一方向で、一度出力したトークンを遡って削除できない。
座標計算を間違えて「Re-do」すると、SVG 内にラベルやグリッド線が二重に残る。

代わりに \`<script>\` 内で動的に生成する:
1. データ配列を \`const\` で定義（ツールから取得した値をそのまま埋め込む）
2. min/max から niceTicks（キリの良い目盛り）を計算
3. ループで Y 軸ラベル・グリッド線の SVG 要素を生成
4. ループでポリライン座標を生成
5. clipPath による二色塗り分けも JS で適用

SVG タグ自体は空の \`<svg id="...">\` だけを出力し、中身はすべて JS で描画する。
SVG コード内で座標を再計算・再出力してはならない。

\`\`\`js
// 実装イメージ
const data = [
  {label:'03/01', value:11063}, {label:'03/02', value:10892},
  // ...
  {label:'現在', value:10565}
];
const svg = document.querySelector('#chart-svg');
const ticks = calcNiceTicks(dataMin, dataMax, 5);
ticks.forEach(val => {
...
- **二色塗り分け**: 基準値（期初の value_jpy）を水平ラインとして、基準値以上の区間は **緑（#4ade80）** の線＋半透明緑の塗り、基準値を下回る区間は **赤（#f87171）** の線＋半透明赤の塗りで描画する。\`<clipPath>\` 等を活用して塗り分ける
...
**レイアウト:**
- **visualizer モード**: 「月初比」「年初比」の **タブ切り替え** で表示する。デフォルトは「月初比」タブを選択状態にする。タブの実装は純粋な HTML + CSS（\`<input type="radio">\` + CSS の \`:checked\` セレクタ）で行い、JavaScript は使わない。ただしグラフ描画部分は \`<script>\` で動的生成する（タブの表示切替と混同しないこと）
  - タブラベルは **font-size: 14px**（0.875rem）、**padding: 8px 16px**、**white-space: nowrap** で折り返さないようにする
  - タブとグラフ本体の間に **12px 以上の余白**（margin-top or padding-top）を確保する
