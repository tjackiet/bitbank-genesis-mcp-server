...
- 期間基準は JST（日本標準時）固定
- 期間中に入出金がある場合は「調整後増減」（入出金元本を除いた成績）も表示
- 入出金の扱い:
  - **出金元本**: 外部フローとして net_flow_jpy に含め、performance から除外
...
- **年初比パフォーマンス**（yearly_performance）— 主指標として目立つ位置に配置
  - 年初評価額 → 現在評価額の変化を表示
  - 入出金がある場合は「調整後増減」（入出金元本を除いた成績）を表示
  - 出金手数料（withdrawal_fee_jpy）がある場合は別行で表示
  - 入出金がない場合は単純増減を表示
