...
- Y 軸ラベル: **実データの JPY 値**（¥XX,XXX 形式）を薄い色（text-gray-500 等）で **グラフの左側**（x=65, text-anchor="end"）に表示。上端（y=25）に maxValue、下端（y=140）に minValue を配置。**ピクセル座標（10, 110 等）を表示してはいけない**
- Y 軸の価格ラベルと X 軸の「月初」「年初」テキストは **30px 以上の垂直距離** を空けること（重ならないようにする）
    ...
    <section class="bg-card rounded-lg p-6">
      ...
      <div class="mb-6">
        ...
        <svg viewBox="0 0 600 190" class="w-full">
          ...
          <!-- Y軸ラベル(x=65, text-anchor=end): 上端(y=25)に ¥{maxValue}, 下端(y=140)に ¥{minValue} -->
          <!-- X軸ラベル(y≈170): 左「月初」右「現在」（Y軸ラベルと30px以上離す） -->
        </svg>
        <div class="flex justify-between text-gray-500 text-xs mt-1">
          <span>月初: {series[0].value_jpy}円</span>
          <span>現在: {series[N-1].value_jpy}円</span>
        </div>
      </div>
      ...
      <div>
        ...
        <svg viewBox="0 0 600 190" class="w-full">
          ...
          <!-- Y軸ラベル(x=65, text-anchor=end): ¥{maxValue} / ¥{minValue}（実データのJPY値） -->
          <!-- X軸ラベル(y≈170): 左「年初」右「現在」 -->
        </svg>
        <div class="flex justify-between text-gray-500 text-xs mt-1">
          <span>年初: {series[0].value_jpy}円</span>
          <span>現在: {series[N-1].value_jpy}円</span>
        </div>
      </div>
    </section>
