...
- 取得時刻（timestamp）
- 年初来実現損益（yearly_realized_pnl / summary の「年初来実現損益」行）
- 月初来実現損益（monthly_realized_pnl / summary の「月初来実現損益」行）
...
【指標の定義と優先順位】
本レポートの主指標は **実現損益（realized PnL）** ベースの期間別損益:
- **年初来実現損益**: 当年 1/1 00:00 JST から現在までの売却による確定損益
- **月初来実現損益**: 当月 1日 00:00 JST から現在までの売却による確定損益
- 期間基準は JST（日本標準時）固定

補助指標として以下も表示するが、主役ではない:
- 評価損益（unrealized_pnl）= 現在保有分の含み損益。約定履歴の平均取得単価ベース
- 全履歴実現損益（total_realized_pnl）= 口座開設以降の累計
...
- **年初来実現損益**（yearly_realized_pnl.realized_pnl）— 主指標として目立つ位置に配置
- **月初来実現損益**（monthly_realized_pnl.realized_pnl）— 主指標として目立つ位置に配置
- 合計評価損益（total_unrealized_pnl）— 補助指標として小さめに表示
- 口座全体リターン（account_return_jpy, account_return_pct）— available 時のみ、補助指標として表示
- プラスは緑、マイナスは赤で色分け
- 売却件数がゼロの場合は「（売却なし）」と表示
  ...
  <div class="max-w-3xl mx-auto space-y-6">
    ...
    <section class="bg-card rounded-lg p-6">
      ...
      <div class="grid grid-cols-2 gap-4 text-center mb-4">
        <div class="bg-accent rounded-lg p-3">
          <p class="text-gray-400 text-xs">年初来実現損益</p>
          <p class="{yearly_color} text-xl font-bold">{yearly_realized_pnl}</p>
          <p class="text-gray-500 text-xs">{yearly_sell_count}件の売却</p>
        </div>
        <div class="bg-accent rounded-lg p-3">
          <p class="text-gray-400 text-xs">月初来実現損益</p>
          <p class="{monthly_color} text-xl font-bold">{monthly_realized_pnl}</p>
          <p class="text-gray-500 text-xs">{monthly_sell_count}件の売却</p>
        </div>
      </div>
      <!-- 補助指標 -->
      <div class="grid grid-cols-3 gap-4 text-center text-sm">
        ...
        <div>
          <p class="text-gray-400 text-xs">全履歴実現損益</p>
          <p class="{realized_color} font-bold">{total_realized_pnl}</p>
        </div>
      </div>
      <p class="text-gray-500 text-xs text-center mt-2">期間基準: JST（日本標準時）</p>
    </section>
    ...
    <section class="bg-card rounded-lg p-6">
      ...
      <div class="text-sm text-gray-400 space-y-2">
        ...
        <ul class="list-disc list-inside space-y-1 ml-2">
          <li>年初来実現損益 = 当年1/1 00:00 JST 〜 現在の売却確定損益</li>
          <li>月初来実現損益 = 当月1日 00:00 JST 〜 現在の売却確定損益</li>
          <li>評価損益 = 現在保有分の含み損益（平均取得単価ベース、参考値）</li>
          <li>評価損益は税額計算の確定値ではありません</li>
        </ul>
