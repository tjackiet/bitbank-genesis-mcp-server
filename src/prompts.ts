...
1. analyze_my_portfolio(include_pnl=true, include_technical=true, include_deposit_withdrawal=true)
   → 口座全体の保有資産・前日比/月初比/年初比パフォーマンス・評価損益・入出金ベースのリターン・テクニカル分析を一括取得
...
- 取得時刻（timestamp）
- 口座合計評価額（total_jpy_value）
- 前日比の口座増減（daily_performance / summary の「前日比」行）
- 年初比の口座増減（yearly_performance / summary の「年初比」行）
- 月初比の口座増減（monthly_performance / summary の「月初比」行）
...
■ 主指標: 資産残高と期間比較
本レポートの中心は **「今いくらあるか」「どれだけ増減したか」**。

1. **資産残高**（total_jpy_value）= 現在の口座合計評価額。最も目立つ位置に表示
2. **前日比**: 当日 00:00 JST 時点の口座評価額 vs 現在（daily_performance）
3. **月初比**: 当月 1日 00:00 JST 時点の口座評価額 vs 現在（monthly_performance）
4. **年初比**: 当年 1/1 00:00 JST 時点の口座評価額 vs 現在（yearly_performance）

各期間カードの主表示は **単純増減（change_jpy / change_pct）** — ユーザーが直感的に把握できる数字。

■ 補助指標: 入出金調整後の増減
期間中に入出金がある場合は補助として表示。ラベル統一:
- **入出金調整後の増減**（adjusted_change_jpy / adjusted_change_pct）
- サブ説明: 「入出金元本を除いた実質的な増減。出金手数料は含む」
入出金の扱い:
  - **出金元本**: 外部フローとして net_flow_jpy に含め、adjusted_change から除外
  - **出金手数料**: コストとして withdrawal_fee_jpy に分離し、adjusted_change_jpy に残る
  - 暗号資産の入出庫は現在価格で仮評価

■ その他の指標
- 評価損益（unrealized_pnl）= 現在保有分の含み損益。約定履歴の平均取得単価ベース。補助的に表示
- 口座全体リターン（account_return）= 入出金ベースの概算。available 時のみ補助表示
- 全履歴実現損益（total_realized_pnl）= **本レポートでは非表示**（ノイズが大きいため）

※ 評価損益は参考値であり、**所得税計算には使用できません**

【出力形式】
取得したデータを使って、以下の構成の **HTML ファイル** を生成してください。
構成の優先順位: **資産残高 → 期間比較 → 資産構成比率 → 銘柄詳細 → テクニカル**
...
### 2. 資産残高セクション（最重要）
これがレポートの主役。最も大きく目立つセクション。

- 口座合計評価額（total_jpy_value）を **大きなフォント（text-4xl）** で中央表示
- うち JPY 残高を小さめに補足
- すぐ下に **前日比** を表示（daily_performance の change_jpy / change_pct）
  - プラスは緑、マイナスは赤で色分け
- 評価損益（total_unrealized_pnl / total_unrealized_pnl_pct）— 小さめの補助表示
- 口座全体リターン（account_return_jpy, account_return_pct）— available 時のみ、小さめに補助表示

### 3. 前日比 / 月初比 / 年初比カード（3カード横並び or 縦並び）
各カードに以下を表示:
- **ラベル**: 「前日比」「月初比」「年初比」
- **期初評価額 → 現在評価額** を小さく表示
- **主表示: 単純増減（change_jpy / change_pct）** — 大きなフォント、プラス緑・マイナス赤
- **入出金調整後の増減**（adjusted_change_jpy / adjusted_change_pct）— 補助表示、小さめ
  - ラベル: 「入出金調整後の増減」
  - サブ説明: 「入出金元本を除いた実質的な増減。出金手数料は含む」
  - 入出金がない期間は非表示でよい
- 純入出金（net_flow_jpy）— 入出金がある場合のみ小さく表示
- 出金手数料（withdrawal_fee_jpy）— 出金手数料がある場合のみ小さく表示
...
**指標の定義:**
- 前日比 = 当日 00:00 JST 時点の口座評価額 vs 現在の口座評価額
- 月初比 = 当月 1日 00:00 JST 時点の口座評価額 vs 現在の口座評価額
- 年初比 = 当年 1/1 00:00 JST 時点の口座評価額 vs 現在の口座評価額
- 期初評価額は約定・入出金を逆算して復元し、期初始値（1dayキャンドル）で評価
- **入出金調整後の増減** = 入出金元本を除いた実質的な増減。出金手数料は含む
- 出金元本は外部フロー（performanceから除外）、出金手数料はコスト（performanceに残る）
- 暗号資産の入出庫は現在価格で仮評価
- 評価損益 = 現在保有分の含み損益（平均取得単価ベース、参考値）
...
<body class="bg-dark text-gray-100 min-h-screen p-6">
  <div class="max-w-3xl mx-auto space-y-6">
    ...
    <section class="bg-card rounded-lg p-6">
      <div class="text-center mb-4">
        <p class="text-gray-400 text-sm">資産残高</p>
      ...
      <div class="text-center mb-4">
        ...
        <p class="{daily_color} text-2xl font-bold">{daily_change_jpy} ({daily_change_pct})</p>
      </div>
      <!-- 補助指標 -->
      <div class="grid grid-cols-2 gap-4 text-center text-sm">
        <div>
          <p class="text-gray-400 text-xs">評価損益（含み）</p>
          <p class="{pnl_color} font-bold">{total_unrealized_pnl}</p>
        </div>
        <div>
          <p class="text-gray-400 text-xs">口座全体リターン</p>
          <p class="{return_color} font-bold">{account_return}</p>
          <!-- available 時のみ表示 -->
        </div>
      </div>
    </section>
    ...
    <!-- 前日比 / 月初比 / 年初比カード -->
    <section class="bg-card rounded-lg p-6">
      <h2 class="font-bold mb-4">📈 期間別パフォーマンス</h2>
      <div class="grid grid-cols-3 gap-4">
        <!-- 前日比カード -->
        <div class="bg-accent rounded-lg p-4 text-center">
          <p class="text-gray-400 text-xs mb-1">前日比</p>
          <p class="text-gray-500 text-xs">{daily_start}円 → {daily_current}円</p>
          <p class="{daily_color} text-xl font-bold mt-1">{daily_change_jpy}</p>
          <p class="{daily_color} text-sm">{daily_change_pct}</p>
          <!-- 入出金がある場合のみ以下を表示 -->
          <div class="mt-2 pt-2 border-t border-gray-600 text-xs text-gray-500">
            <p>入出金調整後の増減</p>
            <p class="{daily_adj_color}">{daily_adjusted}</p>
            <p class="text-gray-600" style="font-size: 10px;">入出金元本を除いた実質的な増減。出金手数料は含む</p>
          </div>
        </div>
        <!-- 月初比カード -->
        <div class="bg-accent rounded-lg p-4 text-center">
          <p class="text-gray-400 text-xs mb-1">月初比</p>
          ...
          <div class="mt-2 pt-2 border-t border-gray-600 text-xs text-gray-500">
            ...
            <p>出金手数料: {monthly_fee}</p>
          </div>
        </div>
        <!-- 年初比カード -->
        <div class="bg-accent rounded-lg p-4 text-center">
          <p class="text-gray-400 text-xs mb-1">年初比</p>
    ...
    <!-- 資産構成比率 -->
    <section class="bg-card rounded-lg p-6">
      <h2 class="font-bold mb-4">📊 資産構成比率</h2>
      <div class="space-y-3">
        <!-- 銘柄ごとの横棒 -->
        <div>
    ...
    <section class="bg-card rounded-lg p-6">
      ...
      <div class="text-sm text-gray-400 space-y-2">
        ...
        <p class="mt-2"><strong>指標の定義:</strong></p>
        <ul class="list-disc list-inside space-y-1 ml-2">
          <li>前日比 = 当日 00:00 JST 時点の口座評価額 vs 現在の口座評価額</li>
          <li>月初比 = 当月1日 00:00 JST 時点の口座評価額 vs 現在の口座評価額</li>
          <li>年初比 = 当年1/1 00:00 JST 時点の口座評価額 vs 現在の口座評価額</li>
          <li>期初評価額は約定・入出金を逆算して復元し、期初始値（1dayキャンドル）で評価</li>
          <li><strong>入出金調整後の増減</strong> = 入出金元本を除いた実質的な増減。出金手数料は含む</li>
          <li>出金元本は外部フロー（performanceから除外）、出金手数料はコスト（performanceに残る）</li>
          <li>暗号資産の入出庫は現在価格で仮評価</li>
          <li>評価損益 = 現在保有分の含み損益（平均取得単価ベース、参考値）</li>
        </ul>
