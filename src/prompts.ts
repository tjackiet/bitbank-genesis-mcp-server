...
- 銘柄別パフォーマンス（holdings_performance）— 各保有銘柄の月初比・年初比の価格騰落率
...
構成の優先順位: **資産残高 → 資産推移グラフ → 銘柄別パフォーマンス → 月初比/年初比 → 資産構成比率 → テクニカル**
...
| 銘柄 | 評価額 | 月初比 | 年初比 |
- **月初比**（monthly_change_pct）= 月初始値 → 現在価格の価格騰落率（%）。ポートフォリオの増減ではなく **その銘柄自体の価格変動**
- **年初比**（yearly_change_pct）= 年初始値 → 現在価格の価格騰落率（%）
- プラスなら緑、マイナスなら赤で色分け
...
- **銘柄別パフォーマンス（月初比/年初比）** = 各銘柄の価格が月初・年初の始値からどれだけ騰落したかの割合（%）。ポートフォリオの増減ではなく、銘柄自体の市場価格の変動率
