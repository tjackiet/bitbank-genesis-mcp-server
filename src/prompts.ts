...
- **Y 軸**: キリの良い数値（500円刻み or 1,000円刻みなど、値域に応じて適切な間隔）で目盛りを配置する。データの min/max をそのまま使わず、切り下げ・切り上げで丸めた値を軸の端にする。¥XX,XXX 形式でグラフ左側に表示。ピクセル座標を表示してはいけない。**目盛りの重複禁止**: 同じ値のラベルを複数回表示しない。目盛りは一定間隔（step）で生成し、目盛り数は **4〜6本** 程度に収める
- **X 軸**: 左に「月初」/「年初」、右に「現在」
