...
- 期間基準は JST（日本標準時）固定
- 期間中に入出金がある場合は「調整後増減」（市場変動のみの成績）も表示
- 入出金の扱い: 純入出金（入金 - 出金）を差し引くことで、入出金による見た目の歪みを補正
...
- **年初比パフォーマンス**（yearly_performance）— 主指標として目立つ位置に配置
  - 年初評価額 → 現在評価額の変化を表示
  - 入出金がある場合は「調整後増減」（入出金影響を除いた市場変動のみ）を表示
  - 入出金がない場合は単純増減を表示
    ...
    <section class="bg-card rounded-lg p-6">
      ...
      <div class="grid grid-cols-2 gap-4 text-center mb-4">
        <div class="bg-accent rounded-lg p-3">
          ...
          <!-- 入出金がある場合は調整後増減を表示 -->
          <p class="text-gray-500 text-xs">{yearly_note}</p>
        </div>
        <div class="bg-accent rounded-lg p-3">
          ...
          <!-- 入出金がある場合は調整後増減を表示 -->
          <p class="text-gray-500 text-xs">{monthly_note}</p>
        </div>
      </div>
      <!-- 補助指標 -->
      ...
      <p class="text-gray-500 text-xs text-center mt-2">期間基準: JST（日本標準時） / 入出金がある場合は調整後増減を表示</p>
    </section>
    ...
    <section class="bg-card rounded-lg p-6">
      ...
      <div class="text-sm text-gray-400 space-y-2">
        ...
        <ul class="list-disc list-inside space-y-1 ml-2">
          ...
          <li>入出金がある場合、調整後増減 = 単純増減 - 純入出金（市場変動のみの成績）</li>
