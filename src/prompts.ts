...
**月次グラフ（月初比資産増減）:**
- monthly_performance の start_value_jpy（月初の資産総額）と current_value_jpy（現在の資産総額）を使う
- 期間: 当月1日 00:00 JST → 現在
- グラフの左端を start_value_jpy、右端を current_value_jpy として折れ線を描画
- グラフ上部に大きく: 月初比資産増減の **change_jpy** と **change_pct** を表示（プラスは緑、マイナスは赤）
- グラフ下部に: start_value_jpy → current_value_jpy を小さく表示

SVG 生成手順（おはようレポートと同じ方式）:
- viewBox="0 0 600 150"
- 描画エリア x: 10〜590、y: 10〜110
- \`<polyline>\` で折れ線、\`<polygon>\` で半透明の面塗り、始点・終点に \`<circle>\` マーカー
- 線の色: 増加は #4ade80（緑）、減少は #f87171（赤）
- X 軸左に「月初」、右に「現在」のラベル
- もし daily_performance 等から中間データポイントが取れる場合はそれも使う。取れなければ 2点（start, current）で直線でよい

**年次グラフ（年初比資産増減）:**
- 同様に yearly_performance の start_value_jpy と current_value_jpy を使う
- 期間: 当年1/1 00:00 JST → 現在
- グラフ上部に大きく: 年初比資産増減の **change_jpy** と **change_pct**
- グラフ下部に: start_value_jpy → current_value_jpy
  ...
  <div class="max-w-3xl mx-auto space-y-6">
    ...
    <section class="bg-card rounded-lg p-6">
      ...
      <div class="mb-6">
        ...
        <!-- インライン SVG 折れ線グラフ: viewBox="0 0 600 150" -->
        <!-- 左端 = start_value_jpy（月初）、右端 = current_value_jpy（現在） -->
        <!-- polyline で折れ線、polygon で半透明面塗り、始点・終点に circle マーカー -->
        <!-- 線色: 増加=#4ade80, 減少=#f87171 -->
        <svg viewBox="0 0 600 150" class="w-full">
          <!-- グラフ描画エリア: x=10〜590, y=10〜110 -->
          <!-- X軸ラベル: 左に「月初」、右に「現在」 -->
        </svg>
        <div class="flex justify-between text-gray-500 text-xs mt-1">
          <span>月初: {monthly_start_value}円</span>
          <span>現在: {monthly_current_value}円</span>
        </div>
      </div>
      ...
      <div>
        ...
        <svg viewBox="0 0 600 150" class="w-full">
          <!-- 同様に start_value_jpy → current_value_jpy -->
        </svg>
        <div class="flex justify-between text-gray-500 text-xs mt-1">
          <span>年初: {yearly_start_value}円</span>
          <span>現在: {yearly_current_value}円</span>
        </div>
      </div>
    </section>
