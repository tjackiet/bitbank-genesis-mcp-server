...
【チャート詳細度（chartDetail）】
- minimal（デフォルト）: エクイティカーブ + ドローダウンのみ。「損益グラフ」「パフォーマンス」等の要望にはこちらで十分。
- full: 価格+インジケーター+エクイティ+DD+ポジションの5段構成。「価格チャート上でエントリー/エグジットを見たい」「MACDの動きを確認したい」等、明示的に価格やインジケーターの可視化を求められた場合のみ使用。
