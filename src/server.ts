...
【チャート詳細度（chartDetail）】
- minimal（デフォルト）: エクイティカーブ + ドローダウンの軽量チャート
- full: 価格チャート + インジケーターパネル + エクイティ + ドローダウン + ポジションの5段構成
  ┌ 価格パネル ── 終値ライン + エントリー(▲)/エグジット(▼)マーカー
  ├ インジケーターパネル ── 戦略に応じた指標（例: MACDライン/シグナル/ヒストグラム）
  ├ エクイティパネル ── 戦略 vs バイ&ホールド
  ├ ドローダウンパネル
  └ ポジションパネル
※ 「チャートを見せて」「エントリー/エグジットを可視化して」等の要望には chartDetail: "full" を使ってください。
