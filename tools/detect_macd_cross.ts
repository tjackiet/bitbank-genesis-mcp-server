...
function diffAt(line: number[], signal: number[], i: number): number | null {
	return (line[i] ?? null) != null && (signal[i] ?? null) != null
		? (line[i] as number) - (signal[i] as number)
		: null;
}

function findPrevCross(line: number[], signal: number[], before: number): { idx: number; type: 'golden' | 'dead' } | null {
...
function detectCrossInRange(
	...
	for (let i = end; i >= start; i--) {
		...
		const retPct = priceAtCross && currentPrice != null
...
async function singlePairMode(
	...
	if (includeForming) {
		...
		for (let i = nowIdx; i >= Math.max(1, nowIdx - 3); i--) {
			...
			const hc = hist[i];
			if (hp != null && hc != null) {
				if (hp <= 0 && hc > 0) { lastCrossIdx = i; lastCrossType = 'golden'; break; }
				if (hp >= 0 && hc < 0) { lastCrossIdx = i; lastCrossType = 'dead'; break; }
			}
		}
...
- limit: 上位N件`,
	inputSchema: z.object({
		...
		performanceWindows: z.array(z.number().int().min(1).max(30)).optional().default([1, 3, 5, 10] as any),
	...
	handler: async (args: any) => {
		try {
			...
			const res: any = await screenMode(args.market ?? 'all', args.lookback ?? 3, args.pairs, args.view ?? 'summary', args.screen);
			if (!res?.ok || args.view !== 'detailed') return res;
			try {
				const detRaw: any[] = Array.isArray(res?.data?.screenedDetailed)
					? res.data.screenedDetailed
					: (Array.isArray(res?.data?.resultsDetailed) ? res.data.resultsDetailed : []);
