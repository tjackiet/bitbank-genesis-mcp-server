...
export function detectDoubles(ctx: DetectContext): DetectResult {
					...
					// サイズ下限フィルタ（3%未満は除外）+ 谷深さフィルタ
					{
						...
						if (valleyDepthPct < 0.05) {
							pcand({
								type: 'double_top',
								accepted: false,
								reason: 'valley_too_shallow_relaxed',
								idxs: [a.idx, b.idx, c.idx],
							});
							continue;
						}
					...
					// サイズ下限フィルタ（3%未満は除外）+ 山高さフィルタ
					{
						...
						if (peakHeightPct < 0.05) {
							pcand({
								type: 'double_bottom',
								accepted: false,
								reason: 'peak_too_shallow_relaxed',
								idxs: [a.idx, b.idx, c.idx],
							});
							continue;
						}
	...
	if (includeForming && (want.size === 0 || want.has('double_top') || want.has('double_bottom'))) {
		...
		if ((want.size === 0 || want.has('double_top')) && allPeaks.length >= 1 && allValleys.length >= 1) {
			...
			const valleyAfterPeak = lastConfirmedPeak
				? allValleys.find((v) => v.idx > lastConfirmedPeak.idx && v.idx < lastIdx - 1)
				: null;
