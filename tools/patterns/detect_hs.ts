...
export function detectHeadAndShoulders(ctx: DetectContext): DetectResult {
	...
	if (includeForming && (want.size === 0 || want.has('head_and_shoulders'))) {
		...
		if (confirmedPeaks.length >= 2) {
			...
			// 左肩: 頭より左のピークで、頭より低い
			const leftCandidates = confirmedPeaks.filter(
				(p) => p.idx < head.idx && head.price > p.price * 1.03, // 頭が左肩より3%以上高い
			);
			...
			if (leftCandidates.length >= 1) {
				...
				if (postHeadValley) {
					...
					if (rightShoulder) {
						// 完成度計算
						const closeness =
							1 - Math.abs(rightShoulder.price - left.price) / Math.max(1e-12, left.price * rightPeakTolerancePct);
	...
	if (includeForming && (want.size === 0 || want.has('inverse_head_and_shoulders'))) {
		...
		if (confirmedValleys.length >= 2) {
			...
			// 左肩: 頭より左の谷で、頭より高い
			const leftCandidates = confirmedValleys.filter(
				(v) => v.idx < head.idx && head.price < v.price * 0.97, // 頭が左肩より3%以上低い
			);
			...
			if (leftCandidates.length >= 1) {
				...
				if (postHeadPeak) {
					...
					if (rightShoulder) {
						// 完成度計算
						const closeness =
							1 - Math.abs(rightShoulder.price - left.price) / Math.max(1e-12, left.price * rightValleyTolerancePct);
						const progress = Math.max(0, Math.min(1, closeness));
						const completion = Math.min(1, (0.75 + 0.25 * progress) * (isProvisional ? 0.9 : 1.0));
