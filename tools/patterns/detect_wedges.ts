...
export function detectWedges(ctx: DetectContext): DetectResult {
  ...
  // 4b（回帰ベース）が完成済みの主力。4d は形成中向け（緩い条件）
  {
    ...
    for (const w of fWindows) {
      ...
      if (!bothDown && !bothUp) {
        formingWedgeDebug.push({ type: (upperLine.slope < 0 ? 'falling_wedge' : 'rising_wedge') as any, accepted: false, reason: 'slopes_not_same_direction', indices: [startIdx, endIdx], details: { slopeU: upperLine.slope, slopeL: lowerLine.slope } });
        continue;
      }
