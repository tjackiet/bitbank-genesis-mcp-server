...
export function detectWedges(ctx: DetectContext): DetectResult {
	...
	// 4b) Revamped Wedge scanning (rising/falling)
	{
		...
		for (const w of windows) {
			...
			if (
				...
				debugCandidates.push({
					...
					details: {
						...
						minRequired: params.minTouchesPerLine ?? 2,
					},
				});
				continue;
			}
			...
			const status4b: 'completed' | 'invalid' | 'near_completion' = breakInfo.detected
				? 'completed'
				: 'near_completion';
	...
	// 4b（回帰ベース）が完成済みの主力。4d は形成中向け（緩い条件）
	{
		...
		for (const w of fWindows) {
			...
			if ((wedgeType === 'falling_wedge' && !fAllowFalling) || (wedgeType === 'rising_wedge' && !fAllowRising))
				continue;
