			...
			if (wantTripleTop && highsOnly.length >= 3) {
				for (let i = 0; i <= highsOnly.length - 3; i++) {
					...
					if (start && end) {
						...
						const valleysNear =
							Math.abs(v1.price - v2.price) / Math.max(1, Math.max(v1.price, v2.price)) <= tolerancePct;
						...
						const tolMargin = clamp01(
							1 - devs.reduce((s, v) => s + v, 0) / devs.length / Math.max(1e-12, tolerancePct),
						);
						...
						if (confidence >= (MIN_CONFIDENCE.triple_top ?? 0)) {
							...
							push(patterns, {
								...
								...(ttTarget !== undefined
									? { breakoutTarget: ttTarget, targetMethod: 'neckline_projection' as const }
									: {}),
					...
					if (start && end) {
						...
						if (confidence >= (MIN_CONFIDENCE.triple_bottom ?? 0)) {
							...
							push(patterns, {
								...
								...(neckline ? { neckline, trendlineLabel: 'ネックライン' } : {}),
								...(tbTarget !== undefined
									? { breakoutTarget: tbTarget, targetMethod: 'neckline_projection' as const }
									: {}),
								...(diagram ? { structureDiagram: diagram } : {}),
							});
							pcand({
			...
			for (const f of [1.25, 2.0]) {
				...
				if (wantTripleTop && !patterns.some((p) => p.type === 'triple_top')) {
					...
					for (let i = 0; i <= hs.length - 3 && !placed; i++) {
						...
						const necklineSlopeLimit = 0.02;
						const necklineSlope = Math.abs(v1.price - v2.price) / Math.max(1, Math.max(v1.price, v2.price));
						if (necklineSlope > necklineSlopeLimit) {
							pcand({
								type: 'triple_top',
								accepted: false,
	...
	if (includeForming && (want.size === 0 || want.has('triple_top') || want.has('triple_bottom'))) {
		...
		if ((want.size === 0 || want.has('triple_bottom')) && allValleys.length >= 2) {
			...
			for (let i = confirmedValleys.length - 1; i >= 1; i--) {
				...
				// 2つの谷が等安か
				const valleyDiff =
					Math.abs(valley1.price - valley2.price) / Math.max(1, Math.max(valley1.price, valley2.price));
