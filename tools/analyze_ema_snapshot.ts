...
import { AnalyzeEmaSnapshotInputSchema, AnalyzeEmaSnapshotOutputSchema } from '../src/schemas.js';
...
export default async function analyzeEmaSnapshot(
  ...
  try {
    ...
    if (hasCustomPeriods) {
      ...
      if (!candlesResult.ok) return AnalyzeEmaSnapshotOutputSchema.parse(fail(candlesResult.summary || 'candles failed', (candlesResult.meta as any)?.errorType || 'internal')) as any;
      ...
      if (!indRes?.ok) return AnalyzeEmaSnapshotOutputSchema.parse(fail(indRes?.summary || 'indicators failed', (indRes?.meta as any)?.errorType || 'internal')) as any;
    ...
    for (const [a, b] of crossPairs) {
      ...
      if (va != null && vb != null) {
        ...
        crosses.push({ a: `EMA_${a}`, b: `EMA_${b}`, type: delta >= 0 ? 'golden' : 'dead', delta: Number(delta.toFixed(2)) });
      }
    }
    ...
    const emasExt: Record<string, { value: number | null; distancePct: number | null; distanceAbs: number | null; slope: 'rising' | 'falling' | 'flat'; slopePctPerBar: number | null; slopePctTotal: number | null; barsWindow: number | null; slopePctPerDay?: number | null; pricePosition?: 'above' | 'below' | 'equal' }> = {};
    for (const p of periods) {
    ...
    const distanceLines = topPeriods.map(p => {
      ...
      return `EMA(${p}): ${valStr} (${pctStr}, ${absStr}) slope=${it?.slope}${slopeRate ? ` (${slopeRate})` : ''}${pos}`;
    });
...
export const toolDef: ToolDefinition = {
	...
	description: '[EMA / Exponential Moving Average] EMA（exponential moving average / trend / slope）の最新値・整列・クロス・傾きを返す（既定: 12/26/50/200）。',
