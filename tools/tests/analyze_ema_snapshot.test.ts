...
import analyzeEmaSnapshot, { toolDef } from '../analyze_ema_snapshot.js';
...
describe('analyze_ema_snapshot', () => {
  ...
  it('analyze_indicators が失敗を返した場合は ok: false を返す', async () => {
    ...
    const res: any = await analyzeEmaSnapshot('btc_jpy', '1day', 220, [12, 26, 50, 200]);
  ...
  it('重複periods指定時は自己クロス（EMA_12/EMA_12）や重複クロスを出さないべき', async () => {
    ...
    const res: any = await analyzeEmaSnapshot('btc_jpy', '1day', 220, [12, 12, 26]);
