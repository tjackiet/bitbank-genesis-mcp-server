...
function makeOhlcvRows(count: number, opts?: {
  baseClose?: number;
  drift?: number;
  noise?: number;
  flat?: boolean;
...
describe('get_volatility_metrics', () => {
  ...
  describe('正常系（30本、annualize=true）', () => {
    ...
    it('ok: true で全必須フィールドを含む', async () => {
      ...
      const res: any = await getVolatilityMetrics('btc_jpy', '1day', 30);
  ...
  describe('annualize: false', () => {
    ...
    it('rv_std_ann が undefined になる', async () => {
      mockFetchWithOhlcv(rows);
      const res: any = await getVolatilityMetrics('btc_jpy', '1day', 30, [14, 20], { annualize: false });
      expect(res.ok).toBe(true);
  ...
  describe('エッジケース', () => {
    ...
    it('ちょうど 20 本（最小）で ok: true', async () => {
      ...
      mockFetchWithOhlcv(rows);
      const res: any = await getVolatilityMetrics('btc_jpy', '1day', 20);
      expect(res.ok).toBe(true);
    ...
    it('window がデータ長を超える場合、rolling から除外される', async () => {
      ...
      mockFetchWithOhlcv(rows);
      const res: any = await getVolatilityMetrics('btc_jpy', '1day', 25, [14, 20, 100]);
      expect(res.ok).toBe(true);
      const windowsInResult = res.data.rolling.map((r: any) => r.window);
  ...
  describe('Parkinson estimator', () => {
    ...
    it('全ローソク足の H/L 比から Parkinson が手計算と一致する', async () => {
      ...
      mockFetchWithOhlcv(rows);
      const res: any = await getVolatilityMetrics('btc_jpy', '1day', 30, [14], { annualize: false });
  ...
  describe('タグ生成', () => {
    ...
    it('calm タグの閾値は 0.3 以下（tool 側）', async () => {
      ...
      mockFetchWithOhlcv(rows);
      const res: any = await getVolatilityMetrics('btc_jpy', '1day', 200, [14, 20, 30], { annualize: true });
  ...
  describe('Handler タグ閾値の annualize=false 問題', () => {
    ...
    it('annualize=false の per-period RV は常に handler 閾値 0.2 未満になる', async () => {
      ...
      mockFetchWithOhlcv(rows);
      const res: any = await getVolatilityMetrics('btc_jpy', '1day', 60, [14, 20, 30], { annualize: false });
      expect(res.ok).toBe(true);
