...
import renderCandlePatternDiagram, { toolDef } from '../render_candle_pattern_diagram.js';
...
describe('render_candle_pattern_diagram', () => {
  it('title と日付ラベルの XML 特殊文字をエスケープできる', async () => {
    const res: any = await renderCandlePatternDiagram({
      candles: [
        makeCandle('01/01<&>', 100, 110, 95, 98, 'bearish'),
        makeCandle('01/02"', 97, 112, 96, 109, 'bullish'),
      ],
  ...
  it('価格レンジが 0 のときも SVG に NaN や Infinity を含めるべきではない', async () => {
    const res: any = await renderCandlePatternDiagram({
      candles: [
        makeCandle('01/01', 100, 100, 100, 100, 'bullish'),
        makeCandle('01/02', 100, 100, 100, 100, 'bearish'),
      ],
      pattern: {
        name: '毛抜き底',
        confirmedDate: '01/02',
