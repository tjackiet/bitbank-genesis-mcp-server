...
vi.mock('../get_candles.js', () => ({
  default: vi.fn(),
}));

import getCandles from '../get_candles.js';
import analyzeSupportResistance, { toolDef } from '../analyze_support_resistance.js';
...
function buildFlatCandles(count: number, pairBase = 100): Candle[] {
  return Array.from({ length: count }, (_, i) =>
    makeCandle(i, pairBase, pairBase + 1, pairBase - 1, pairBase, 100)
  );
}
...
function buildBufferedHistoryCandles(): Candle[] {
  ...
  const recentWindow = Array.from({ length: 30 }, (_, i) =>
    makeCandle(10 + i, 100, 101, 99, 100, 100)
  );
...
describe('analyze_support_resistance', () => {
  ...
  it('content の見出しは入力 pair を使うべき', async () => {
    ...
    const res: any = await analyzeSupportResistance('eth_jpy', { lookbackDays: 30, topN: 3, tolerance: 0.015 });
  ...
  it('lookbackDays 外の取得バッファにだけ存在する水準を結果へ混ぜないべき', async () => {
    ...
    const res: any = await analyzeSupportResistance('btc_jpy', { lookbackDays: 30, topN: 3, tolerance: 0.015 });
