...
| 11 | 中級 | 🌅 おはようレポート（直近8時間） | get_ticker<br>get_candles<br>get_flow_metrics<br>analyze_support_resistance<br>get_orderbook_pressure | 直近8時間の価格変動・急変イベント・サポレジ接触・板状況をファクトベースで報告 | 1. 価格の動き<br>2. 主なイベント<br>3. 出来高・売買バランス<br>4. 重要ラインとの関係<br>5. 今の板状況 |
...
- **ファクトベース**: テクニカル指標の解釈は含めず、事実の整理に特化
- **5つの視点**: 価格変動・急変イベント・売買バランス・サポレジ・板状況
- 詳細な分析は「中級：主要指標でBTCを分析して」と組み合わせて使用
