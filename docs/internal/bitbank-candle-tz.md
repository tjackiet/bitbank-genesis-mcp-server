...
4. もし将来的に `date=YYYYMMDD` を「ユーザー指定タイムゾーンにおける暦日」として再定義するなら、それは**別 PR の仕様変更**として扱う。その場合は sub-day 系（1min/5min/15min/30min/1hour）が UTC 2 日にまたがるため、要求 `YYYYMMDD` と前日 `YYYYMM(DD-1)` の 2 chunk を fetch して anchor 端で filter する必要が出てくる。
