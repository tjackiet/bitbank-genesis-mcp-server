...
### フェーズ 4: テストカバレッジ強化 — 見送り（既存テストで十分）
...
当初計画していた以下の項目は、現時点では見送る。

1. **プロパティベーステスト**（`fast-check` 等）
   - メリット: 手書きでは網羅しきれない境界値（負値・極大値・NaN 混在等）を自動発見できる
   - 見送り理由: 新規依存の追加コスト、テスト実行時間増。既存の決定的テストが主要プロパティを暗黙にカバーしている

2. **スナップショットテスト**（vitest snapshot）
   - メリット: リファクタ時に小数点以下の丸め変化など、許容範囲内に収まる意図しない出力変化を検出できる
   - 見送り理由: スナップショット管理コスト、意図的な計算変更時にノイズとなる

**再検討トリガー**: 指標ロジックの大規模リファクタを実施する際に、回帰検出の安全網として導入を再検討する
