...
│  prepare_chart_data   描画向け整形（時系列）   │
│  prepare_depth_data   描画向け整形（板）       │
└─────────────────────────────────────────────┘
          ↓ 計算済みデータ（シフト適用済み）
┌─────────────────────────────────────────────┐
...
### 1. クライアント側描画パス（`prepare_chart_data` / `prepare_depth_data`）

Claude Desktop の Visualizer や、Skills（Artifacts 等）対応クライアント向け。
計算済みのデータを返し、描画はクライアントに委ねる。

- **対象**: Claude Desktop、Skills 対応クライアント
- **利点**: クライアントがインタラクティブなチャートを描画できる。LLM も数値を直接参照可能
- **ツール**: `prepare_chart_data`（時系列）, `prepare_depth_data`（板の累積 volume）
...
- **対象**: Skills 非対応クライアント、軽量モデル利用時、ファイル保存用途
